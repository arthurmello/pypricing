"""IV recovery: associational fit is biased; control-function IV recovers ε."""
...
def _fit(df, panel_columns: PanelColumns, seed: int):
    model = LogLogDemandModel(panel_columns=panel_columns)
    model.fit(
        df,
...
def _true_elasticity(model: LogLogDemandModel, truth) -> np.ndarray:
...
def test_endogenous_prices_bias_associational_elasticity():
    ...
    assert coverage < 0.5, f"associational HDI still covered truth; coverage={coverage:.2f}"


def test_iv_recovers_elasticity_under_endogenous_prices():
    df, truth = _endogenous_panel(seed=21)
